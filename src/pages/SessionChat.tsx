...
const SessionChat = () => {
  ...
  useEffect(() => {
    ...
    const fetchSessionData = async () => {
      const {
        data: sessionData,
        error: sessionError
      } = await (supabase as any).from("sessions").select("*").eq("id", id).single();
      ...
      } = await (supabase as any).from("session_participants").select(`
          *,
      ...
      if (sessionData.creator_id === user?.id) {
        const {
          data: alertsData
        } = await (supabase as any).from("intrusion_attempts").select("*").eq("session_id", id).order("attempt_timestamp", {
          ascending: false
        }).limit(5);
    ...
    fetchSessionData();
    const channel = supabase.channel(`session-${id}`).on("postgres_changes", {
      event: "*",
      schema: "public",
      table: "sessions",
      filter: `id=eq.${id}`
  ...
  return <div className="min-h-screen gradient-animated flex flex-col">
      ...
      <AlertDialog open={showDestroyDialog} onOpenChange={setShowDestroyDialog}>
        <AlertDialogContent className="glass border-destructive/50">
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={handleDestroySession} disabled={isDestroying} className="bg-destructive text-destructive-foreground hover:bg-destructive/90">
      ...
      <header className="glass border-b border-border/50 p-4">
        <div className="flex items-center justify-between max-w-screen-xl mx-auto">
          <div className="flex items-center gap-3">
            <Button variant="ghost" size="icon" onClick={() => navigate("/sessions")} className="hover:bg-primary/10">
            ...
            <div className="flex items-center gap-2 glass rounded-xl px-3 py-2 border border-primary/30">
              ...
              <Button variant="ghost" size="icon" className="h-6 w-6" onClick={copySessionKey}>
          ...
          <div className="flex items-center gap-3">
            ...
            {session?.creator_id === user?.id && <Button variant="destructive" size="sm" onClick={() => setShowDestroyDialog(true)} className="gap-2">
                <X className="w-4 h-4" />
                End Session
              </Button>}
            <Sheet>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 max-w-screen-xl mx-auto w-full">
        {messages.map(msg => <div key={msg.id} ref={el => {
        if (el) messageRefs.current.set(msg.id, el);
      }} data-message-id={msg.id} data-sender-id={msg.sender_id} className={`flex ${msg.sender_id === user?.id ? "justify-end" : "justify-start"} animate-fade-in`}>
            <div className={`max-w-[70%] rounded-2xl p-3 space-y-1 ${msg.sender_id === user?.id ? "bg-primary text-primary-foreground elegant-glow-sm" : "glass"}`}>
              {msg.sender_id !== user?.id && <p className="text-xs font-semibold opacity-70">
                  {msg.sender?.display_name}
                </p>}
              <p className="text-sm whitespace-pre-wrap">{msg.content}</p>
              {msg.chat_mode === 'self-destruct' && <p className="text-xs opacity-50 flex items-center gap-1">
                  💣 Self-destruct
                </p>}
              <div className="flex items-center gap-2 text-xs opacity-70">
                <span>{new Date(msg.created_at).toLocaleTimeString()}</span>
                {msg.sender_id === user?.id && <span>
      ...
      <div className="glass border-t border-border/50 p-4 space-y-3">
        <div className="max-w-screen-xl mx-auto space-y-3">
          {intrusionAlerts.length > 0 && session?.creator_id === user?.id && <div className="glass border border-destructive/50 rounded-xl p-3 animate-fade-in">
        ...
        <div className="max-w-screen-xl mx-auto flex items-center gap-2">
          ...
          <Input value={message} onChange={handleInputChange} onKeyPress={e => e.key === "Enter" && handleSendMessage()} placeholder="Type a secure message..." className="flex-1 glass border-primary/30" disabled={isSending} />
          <Button size="icon" className="elegant-glow" onClick={handleSendMessage} disabled={isSending || !message.trim()}>
